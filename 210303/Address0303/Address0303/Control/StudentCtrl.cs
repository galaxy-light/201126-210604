...
namespace AddressTest0222.control
{
    class StudentCtrl
    {
        ...
        // 기본은 인스턴스임
        // static 삭제 : 인스턴스화
        List<Student> addrList = new List<Student>(); // 모델 클래스를 관리하기 위해 List 사용 / 초기화
        ...
        static StudentCtrl inst; // 클래스 변수

        public static StudentCtrl getInst()
        {
            if (inst == null) // inst가 비어져있을 때
            {
                inst = new StudentCtrl(); // 최초 한번만(inst가 동작할 때 처음은 비어져있음) 객체 생성
            }
        ...
        {            
            ...
            {
                addrList.Add(new Student(rand.getId(), rand.getName(), rand.getTel(),
