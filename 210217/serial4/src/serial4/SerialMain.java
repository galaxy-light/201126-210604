...
//		return menu;
	...
	public static void main(String[] args) {
		// 라이브러리 연결 : Properties -> Java Build Path -> Classpath -> 외부 라이브러리 추가
		...
		while (true) {
			...
			case TV_ON:
				...
				try {					
					serial.writeInt(CMD_TV_ON); // 명령어 전송(문자로 정보를 보냄)
				} catch (SerialPortException e) {
			...
			case TV_OFF:
				...
				// serial = serialPort
				try {					
					serial.writeInt(CMD_TV_OFF); // 명령어 전송(문자로 정보를 보냄)
				} catch (SerialPortException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
			...
			case HEATER_ON:
				...
				// serial = serialPort
				try {
					serial.writeInt(CMD_HEATER_ON); // 명령어 전송(문자로 정보를 보냄)
				} catch (SerialPortException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
			...
			case CLEANER_ON:
				...
				// serial = serialPort
				try {
					serial.writeInt(CMD_CLEANER_ON); // 명령어 전송(문자로 정보를 보냄)
				} catch (SerialPortException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
