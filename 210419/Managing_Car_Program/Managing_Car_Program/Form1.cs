        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    {
                        MessageBox.Show("해당 공간에는 이미 차량이 있음. " + "공간 번호 : " + textBox1.Text);
                        writeLog("해당 공간에는 이미 차량이 있음. " + "공간 번호 : " + textBox1.Text);
                    }
                    else // 아직 차량 정보가 없음
                    {
                        // 참조 변수
            ...
            try
            {
                for (int i = 0; i < DataManager.Cars.Count; i++)
                {
                    if (DataManager.Cars[i].parkingSpot.ToString() == textBox1.Text)
                    {
                        ...
                        {
                            MessageBox.Show("아직 주차 공간에 차량이 없음.");
                            writeLog("아직 주차 공간에 차량이 없음.");
                            break;
                        }
                        else
                        {
                            ...
                            string contents = $"주차공간 {textBox1.Text}에 {textBox2.Text}차량 출자함.";
                            //MessageBox.Show(contents);
                            writeLog(contents);
